    ...
    pub async fn run(
        ...
        let exit_reason_result = if let Some(exit_reason) = pre_loop_exit_reason {
            ...
            loop {
                ...
                static __LOOP_COUNT_155176: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(0);
                let __iter = __LOOP_COUNT_155176.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                ...
                // [codex-codemod] select! loop diagnostics + cooperative yield
                static __LOOP_COUNT_155166: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(0);
                let __iter = __LOOP_COUNT_155166.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                // Yield to JS event loop every 64 iterations to prevent
                // Draw events from monopolizing the select loop. Without
                // this, response animations send hundreds of Draw events
                // that the select processes without ever yielding, blocking
